...
DIR = r"C:\PythonPrograms\gset\midi\clean_midi"
...
instrument_counts = {}
for v in instrument_names.values():
    instrument_counts[v] = 0
        ...
        for track in mf.tracks:
            for message in track:
                ...
                if m[0] == "control_change":
                    instrument_counts[instrument_names[int(m[2][8:])]] += 1
    break

with open("instruments.txt","w") as out:
    out.writelines([f"{k}:{v}\n" for k,v in instrument_counts.items() if v > 0])
# print(*[(k,v) for k,v in instrument_counts.items() if v > 0],sep="\n")
